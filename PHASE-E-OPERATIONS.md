...
The dedicated bearer-authenticated `/api/internal/operations` worker claims up to two due notices per run. A crash or ambiguous provider response is held as uncertain and must be reconciled before an operator decides to retry. Explicit rate limits can retry up to five attempts. More than three receipts to an inbox in an hour are held for review. A separate `?delivery=1` invocation checks one accepted notice through the existing SendGrid Email Activity API, allowing 45 seconds for this lookup. Actual queries varied from 10 to 41 seconds during activation. Sending and reconciliation have independent execution budgets within the route's 60-second limit; the VPS calls them sequentially under its worker lock. A separate delivery check-in detects a stopped reconciler. The existing account event webhook is preserved. Acceptance is not delivery; delivery is receiving-server acceptance, not reading or a shop reply. `delivered_at` records when the worker confirmed delivery evidence.
...
## September 17 activation

The owner designated support@vicrez.com for staff alerts and one labeled delivery test. The outbox was empty before activation. The sole test notice, DELIVERY-TEST-20260917, was accepted once and SendGrid reported delivered to the designated recipient at 17:54:26 UTC. No application, claim or customer inquiry was created for that test. Production activation sets DIRECTORY_NOTIFICATIONS_ENABLED=1 and DIRECTORY_STAFF_ALERT_EMAILS=support@vicrez.com. Daily alerts contain actionable aggregate counts; the existing two-minute worker sends them.

The correct GA4 account is Vicrez Driver (333676956), property installers.vicrez.com (552886038), web stream 15736814286 and tag G-Z5S0TD7F1D. The Analytics UI confirmed active collection. Reports must filter to this stream and hostname installers.vicrez.com. Enhanced measurement is disabled. Directory discovery actions are stored in the first-party operations database, not forwarded as GA4 custom events. GA4 historical data starts September 7, so two complete comparable weeks are not yet available. The September 15 initialization fix is another comparison caveat.

Twenty pilot candidates still await owner confirmation. Candidate selection is not consent to participate. Use the reviewable outreach and qualification worksheet in the private operations workspace before sending invitations or publishing owner details. Two saved directory inquiries and two save events reconcile as of September 17; provider acceptance of four notices does not establish delivery, response, quotes or bookings.
