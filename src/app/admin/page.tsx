...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <a
        href="/admin/actions"
        className="block rounded-xl border border-blue-700 bg-gray-900 p-5 text-blue-300 underline"
      >
        Open the staff action queue for reviews, inquiries and follow-ups
      </a>
      {/* Stats grid */}
            ...
            {stats.topStates.map((item) => {
              ...
              return (
                <div key={item.state} className="flex items-center gap-3">
                  <span className="w-8 text-sm text-gray-500 flex-shrink-0 font-mono">
                    {item.state}
                  </span>
            ...
            {stats.topCapabilities.map((cap) => {
              ...
              return (
                <div key={cap.name} className="flex items-center gap-3">
                  <span
                    className="w-36 text-sm text-gray-500 flex-shrink-0 truncate"
                    title={cap.name}
                  >
                    {cap.name}
                  </span>
            ...
            <tbody>
              {stats.recentAdditions.map((inst) => (
                <tr key={inst.id} className="border-b border-gray-800/50">
                  ...
                  <td className="py-2.5 px-3 text-gray-500">
                    {inst.date_added
                      ? new Date(inst.date_added).toLocaleDateString()
                      : "-"}
                  </td>
                </tr>
              ))}
