  ...
  try {
    let location =
      options.lat !== null && options.lng !== null
        ? { lat: options.lat, lng: options.lng, label: "Your location" }
        : null;
    ...
    const counts = await db.query(
      cte +
        " SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE tier='verified')::int AS verified FROM matches",
      values,
    );
    const ordering =
      options.sort === "nearest" && location
        ? "distance ASC NULLS LAST, id ASC"
        : "(tier='verified') DESC, distance ASC NULLS LAST, id ASC";
