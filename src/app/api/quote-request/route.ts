  ...
  try {
    if (!sameOrigin(request))
      return NextResponse.json(
        { error: "Open the form on this site to continue." },
        { status: 403, headers },
      );
    ...
    if (
      body?.project_detail &&
      body.project_detail.length + (body?.additional_notes || "").length > 480
    )
      return NextResponse.json(
        {
          error:
            "Keep project details and additional notes under 480 characters total.",
        },
        { status: 400, headers },
      );
    if (invalid)
    ...
    if (!(await withinRateLimit(ip, "quote-submit", 12, 900)))
      return NextResponse.json(
    ...
    if (body.installer_id) {
      ...
      if (!rows[0] || !canReceiveQuote(rows[0]))
        return NextResponse.json(
          {
            error:
              "This shop is not accepting requests through Vicrez. Use its phone or website, or choose another installer.",
    ...
    const payload = {
      ...
      notes: [
        body.project_detail ? "Project: " + body.project_detail : "",
        body.additional_notes || "",
      ]
        .filter(Boolean)
        .join("\n"),
      zip_code: body.zip_code || shopZip || null,
      preferred_installer_id: body.installer_id || null,
      flow: body.installer_id ? "selected" : "network",
      sharing_consent: body.sharing_consent,
      consent_version: "directory-quote-2026-09-15",
      install_timeline: body.install_timeline || "",
    ...
    if (response.status === 400 || response.status === 422)
      return NextResponse.json(
        {
          error:
            "Check your contact, vehicle, service and sharing details before trying again.",
        },
        { status: 400, headers },
      );
    if (response.status === 409)
      return NextResponse.json(
        {
          error:
            "This reference already belongs to a different request. Keep your previous receipt and start a new request for changed details.",
    ...
    return NextResponse.json(
      {
        error:
          "We could not confirm your request. Keep the form open and retry the same details; a retry will not create a second request.",
        code: "unconfirmed",
      },
      { status: 503, headers },
    );
