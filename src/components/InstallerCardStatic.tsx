...
export default function InstallerCardStatic({
  installer,
}: InstallerCardStaticProps) {
    ...
    >
      ...
      >
        {isVerified ? (
          ...
          <>
            <svg
              className="w-4 h-4 text-vicrez-muted flex-shrink-0"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
      ...
      <div className="p-4 space-y-3">
        ...
        <h3 className="text-lg font-bold text-white leading-tight">
          {installer.slug ? (
            <a
              ...
              className="hover:text-vicrez-red transition-colors"
            >
        ...
        {displayRating && (
          <div className="flex items-center gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <svg
                key={i}
                className={`w-4 h-4 ${i < Math.round(displayRating) ? "text-yellow-400" : "text-gray-600"}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
        ...
        <div className="flex items-start gap-2 text-sm text-gray-300">
          <svg
            className="w-4 h-4 text-vicrez-muted mt-0.5 flex-shrink-0"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      <div className="px-4 pb-4 pt-2 border-t border-vicrez-border">
        <div className="flex gap-2">
          ...
          {installer.website && (
            <a
              href={
                installer.website.startsWith("http")
                  ? installer.website
