...
export function validateQuoteInput(body: Record<string, any>) {
  ...
  if (!isQuoteService(body.service)) return 'Choose the installation service you need.';
  if (body.sharing_consent !== true) return 'Confirm that you want to share this request with the described recipient(s).';
  if (typeof body.request_id !== 'string' || !UUID.test(body.request_id)) return 'Invalid request reference. Reopen the form.';
  if (body.session_id != null && (typeof body.session_id !== 'string' || !UUID.test(body.session_id))) return 'Invalid session reference.';
  ...
  if (!validBusinessEmail(body.customer_email)) return 'Enter a valid email address.';
  ...
  if (!['string','number'].includes(typeof body.vehicle_year)) return 'Enter a valid vehicle year.';
  ...
  if (body.installer_id != null && (typeof body.installer_id !== 'string' || !body.installer_id.trim() || body.installer_id.length > 80)) return 'Select a valid installer.';
  if (body.zip_code && (typeof body.zip_code !== 'string' || !/^\d{5}(?:-\d{4})?$/.test(body.zip_code))) return 'Enter a valid US ZIP code.';
  ...
  for (const key of ['install_timeline', 'budget_range', 'how_heard']) {
    if (body[key] != null && (typeof body[key] !== 'string' || body[key].length > 120 || /[\u0000-\u001f\u007f]/.test(body[key]))) return 'Check your request details.';
  }
...
export function quoteReceipt(data: Record<string, any>) {
  ...
  if (typeof data.receipt_token !== 'string' || data.receipt_token.length > 512) throw new Error('Receipt status is unavailable');
  return {
    success: true, status: 'received', reference: 'VZ-' + data.submission_id, token: data.receipt_token,
    message: 'Your request is saved. We are checking the selected recipient or matching service providers. Keep this reference; delivery and shop availability are not yet confirmed.',
  };
}
