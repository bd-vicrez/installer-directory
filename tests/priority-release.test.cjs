...
const request = (url, body) => new NextRequest('https://example.test' + url, body === undefined ? {} : { method: 'POST', body: typeof body === 'string' ? body : JSON.stringify(body), headers: { 'content-type': 'application/json' } });
test('public projection retains shop fields and excludes every private field', () => {
  ...
  const projected = toPublicInstaller({ id: 1, business_name: 'Example shop', phone: '555-0100', source: 'manual', email: 'private@example.test', internal_notes: 'secret', harvested_email: 'hidden@example.test', unexpected_future_field: 'secret' });
  assert.equal(projected.business_name, 'Example shop'); assert.equal(projected.phone, '555-0100');
  for (const field of ['email', 'source', 'internal_notes', 'harvested_email', 'unexpected_future_field']) assert.equal(field in projected, false);
});
test('search rejects unbounded pages and malformed location/filter values', () => {
  ...
  for (const query of ['limit=99999', 'offset=-1', 'limit=1.5', 'lat=90', 'lat=91&lng=0', 'radius=0', 'service=unknown', 'q=' + 'x'.repeat(121)]) assert.throws(() => readSearchOptions(new URLSearchParams(query)));
});
test('internal feed rejects anonymous and invalid callers before database access', async () => {
  ...
  const { GET } = load('app/api/internal/installers/route.ts', { '@/lib/db': { getPool() { throw new Error('Database must not be accessed'); } } });
  for (const authorization of ['', 'Bearer wrong']) {
    const result = await GET(new NextRequest('https://example.test/api/internal/installers', { headers: { authorization } }));
...
test('internal feed permits the authorized reader and prevents shared caching', async () => {
  process.env.INSTALLER_FEED_TOKEN = 'a'.repeat(48);
  const { GET } = load('app/api/internal/installers/route.ts', { '@/lib/db': { getPool: () => ({ query: async () => ({ rows: [{ id: 1, email: 'private@example.test' }] }) }) } });
  const result = await GET(new NextRequest('https://example.test/api/internal/installers', { headers: { authorization: 'Bearer ' + process.env.INSTALLER_FEED_TOKEN } }));
  assert.equal(result.status, 200); assert.equal((await result.json())[0].id, 1); assert.match(result.headers.get('cache-control'), /private, no-store/);
});
test('public search uses bounded database queries and projects the result', async () => {
  const calls = [];
  const { GET } = load('app/api/installers/route.ts', { '@/lib/db': { getPool: () => ({ query: async (sql, values) => { calls.push({ sql, values }); return { rows: calls.length === 1 ? [{ total: 2, verified: 1 }] : [{ id: 1, source: 'manual', internal_notes: 'hidden', email: 'hidden' }] }; } }) }, '@/lib/geocode': { geocodeLocation: async () => ({ lat: 34.05, lng: -118.25, label: 'Los Angeles, CA' }) } });
  const result = await GET(request('/api/installers?q=Los%20Angeles&service=PPF&limit=12&offset=12'));
  assert.equal(result.status, 200); const data = await result.json();
  assert.equal(data.total, 2); assert.equal(data.installers[0].internal_notes, undefined);
  assert.match(calls[1].sql, /LIMIT \$\d+ OFFSET \$\d+/); assert.equal(calls[1].values.at(-1), 12);
  assert.ok(calls[0].values.some(v => Array.isArray(v) && v.includes('%ppf%')));
});
test('search distinguishes unknown locations from backend failures', async () => {
  for (const [geocodeLocation, status] of [[async () => null, 422], [async () => { throw new Error('offline'); }, 503]]) {
    const { GET } = load('app/api/installers/route.ts', { '@/lib/geocode': { geocodeLocation } });
    assert.equal((await GET(request('/api/installers?q=00000'))).status, status);
  }
});
test('admin sessions require a signature, expire, and reject legacy or tampered values', () => {
  process.env.ADMIN_SECRET = 's'.repeat(48); process.env.ADMIN_USERNAME = 'admin';
  ...
  const changed = Buffer.from(JSON.stringify({ ...JSON.parse(Buffer.from(payload, 'base64url')), exp: Date.now() + 999999999 })).toString('base64url');
  assert.equal(auth.verifyAdminToken(changed + '.' + signature), false);
  const expired = Buffer.from(JSON.stringify({ user: 'admin', exp: Date.now() - 1 })).toString('base64url');
  const sig = require('node:crypto').createHmac('sha256', process.env.ADMIN_SECRET).update(expired).digest('base64url');
...
test('quote endpoint rejects unconfirmed delivery and retains the same retry identifier', async () => {
  let forwarded;
  const { POST } = load('app/api/quote-request/route.ts', {}, { fetch: async (_url, opts) => { forwarded = JSON.parse(opts.body); return new Response('{}', { status: 502 }); } });
  const result = await POST(request('/api/quote-request', validQuote()));
  assert.equal(result.status, 503); assert.equal(forwarded.request_id, validQuote().request_id); assert.equal((await result.json()).success, undefined);
});
...
test('quote endpoint rejects malformed or oversized bodies before any delivery', async () => {
  const { POST } = load('app/api/quote-request/route.ts', {}, { fetch: async () => { throw new Error('Must not send'); } });
