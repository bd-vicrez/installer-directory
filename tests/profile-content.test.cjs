...
const {
  ...
  getProfileQuoteNote,
  getProfileGuide,
  REVIEWED_PROFILE_SLUGS,
  serializeJsonLd,
} = require("../src/lib/profile-content.ts");
const { generateInstallerJsonLd } = require("../src/lib/seo.ts");
const shop = {
  id: 1,
  slug: "b-wraps-las-vegas-nv",
  business_name: "B wraps",
  city: "Las Vegas",
  state: "NV",
  source: "[New Dealer Form]",
  status: "active",
  install_capabilities: ["Aero/Body Kits", "Paint/Bodywork"],
  phone: "555-0100",
  google_phone: "555-0199",
  website: "shop.example",
  google_website: "other.example",
  internal_notes: "rating: 5",
};
test("pilot contains exactly ten unique profiles", () =>
  assert.equal(new Set(REVIEWED_PROFILE_SLUGS).size, 10));
test("description uses recorded services, not assumed wraps or PPF", () => {
  const text = getProfileDescription(shop);
  assert.match(
    text,
    /Listed services: Body kits & aero parts, Paint & bodywork/,
  );
...
test("legacy delimited capability strings remain supported", () =>
  assert.match(
    getProfileDescription({
      ...shop,
      install_capabilities: "Vinyl Wrap;Paint/Bodywork",
    }),
    /Paint & bodywork, Vinyl wrap/,
  ));
test("quote notes are only shown for eligible pilot dealer-form records", () => {
  ...
  for (const changed of [
    ...
    assert.equal(getProfileQuoteNote({ ...shop, ...changed }), null);
});
test("guide links are grounded in recorded services", () => {
  assert.equal(
    getProfileGuide(shop).href,
    "/guides/how-to-choose-body-kit-installer",
  );
  assert.equal(getProfileGuide({ ...shop, install_capabilities: [] }), null);
  assert.equal(
    getProfileGuide({ ...shop, install_capabilities: ["Widebody Kits"] }).href,
    "/guides/widebody-kit-installation-guide",
  );
});
...
test("schema does not fabricate aggregate rating, prices, shop image or parsed hours", () => {
  ...
  for (const key of [
    ...
    assert.equal(schema[key], undefined);
});
